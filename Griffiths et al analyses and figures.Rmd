...
summary<-ddply(ang5, .(site, sample_time, group.y), summarise, min((ratio)))
...
## august 2019: we have 4-5 samples on a transect; transects were not permanenet, but sample a site. each site was sampled 3 times. sites are either 'interior' or 'edge'. check variation at sites by date. it's looking like not much variation among inner vs edge sites; check a model for that, pooled over time?

smith.load.date <- ggplot(ang5, aes(x = group.y, y = ratio, group_by(sample_time))) + 
  geom_jitter(position = position_jitter(width=.2), size=3, colour = "gray60") +
  stat_summary(fun.data = min.mean.sd.max, geom = "boxplot", fill = "transparent") + 
  scale_y_continuous(name = "Smithora (g dwt) / Zostera (g dwt)", limits = c(-0.15, 4), breaks = c(0,1,2,3,4), labels = c(0,1,2,3,4)) + 
  theme_bw() + 
  facet_grid(cols = vars(sample_time), rows = vars(site)) +
  theme(panel.grid = element_blank()) + 
  xlab(expression("Location")) + 
  geom_text(label = "C", x = 2.4, y = 4) +
  geom_text(label = "C", x = 2.4, y = 4) +
