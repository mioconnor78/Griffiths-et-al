...
## august 2019: we have 4-5 samples on a transect; transects were not permanenet, but sample a site. i just realized i have to nest the shoots within transects! need to redo models below.

smith.load.date <- ggplot(ang5, aes(x = sample_time, y = logratio, group_by(group.y))) + 
  geom_jitter(position = position_jitter(width=.2), size=3, colour = "gray60") +
  stat_summary(fun.data = min.mean.sd.max, geom = "boxplot", fill = "transparent") + 
  #geom_point() +
  scale_y_continuous(name = "log(Smithora (g dwt) / Zostera (g dwt))", limits = c(-2.5, 1.5), breaks = c(-2.5,-2, -1.5,-1,-0.5,0,0.5,1,1.5), labels = c(-2.5,-2, -1.5,-1,-0.5,0,0.5,1,1.5)) + 
  theme_bw() + 
  facet_grid(cols = vars(group.y)) +
  theme(panel.grid = element_blank()) + 
  xlab(expression("Sample period")) + 
 # geom_text(label = "C", x = 2.4, y = 4) +
 # geom_text(label = "C", x = 2.4, y = 4) +
  theme(axis.title.x = element_text(vjust = -1, size = 12)) + 
  theme(axis.title.y = element_text(vjust = -1, size = 12))
smith.load.date
ggsave("smith.load.date.jpg", plot = smith.load.date, width = 6, height = 3)
```
