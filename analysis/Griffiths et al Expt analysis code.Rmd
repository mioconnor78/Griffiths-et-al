...
```{r shoots by dry mass regression}
...
```{r invertebrate data processing} 
...
```{r}
grazer.zost <- ggplot(combined, aes(x = Zone.y, y = gpgG)) + 
  geom_jitter(position = position_jitter(width=.2), size=3, colour = "gray60") +
...
## Figure A1: additional invert plots
```{r invert plots}
...
## Figure: Smithora load / Grazer load
