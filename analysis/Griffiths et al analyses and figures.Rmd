...
ang <- read.csv("../data/macrophyte_biomass_zeros_20170709_smneg.csv") # I manually added small values to smithora dry weights when 0 was entered but smithora was noted in the notes column
...
ang.zostera$shoot_width_cm[ang.zostera$shoot_width_cm == "10"] <- "1.0"

ang.zostera <- ang.zostera %>%
 mutate(LAI = 2*(shoot_length_cm * as.numeric(shoot_width_cm)) * blades) 

ang4 <- merge(ang.smith, ang.zostera, by = c("blade_sample_id", "year","sample_time", "site", "distance_m"), all = TRUE)
ang5 <- ang4[, c("site", "group.y", "sample_time", "blade_sample_id", "distance_m", "macrophyte.x","final_dry_g.x", "macrophyte.y", "final_dry_g.y", "LAI")]
ang5 <- ang4 %>%
  select(c(site, group.y, sample_time, blade_sample_id, distance_m, macrophyte.x,final_dry_g.x, macrophyte.y, final_dry_g.y, LAI)) %>%
...
ang5 <- ang5 %>%
mutate(ratio = as.numeric(ang5$final_dry_g.x) / as.numeric(ang5$final_dry_g.y)) %>%
  mutate(logratio = log(ang5$ratio + 0.1)) %>%
 mutate(load.LAI = as.numeric(final_dry_g.x) / LAI)
...
```{r Figure 1C smithora load, echo=FALSE}
...
Figure A2a: Smithora load.LAI in edge v interior sites  
```{r Figure A2a smithora load LAI, echo=FALSE}
smith.load.LAI <- ggplot(ang5, aes(x = group.y, y = load.LAI)) + 
  geom_jitter(position = position_jitter(width=.2), size=3, colour = "gray60") +
  stat_summary(fun.data = min.mean.sd.max, geom = "boxplot", fill = "transparent") + 
  scale_y_continuous(name = expression(paste("Smithora (g dwt) / Zostera LAI (", cm^2, ")"))) + 
  theme_bw() + 
  theme(panel.grid = element_blank()) + 
  xlab(expression("Location")) + 
  #geom_text(label = "C", x = 2.4, y = 4) +
  theme(axis.title.x = element_text(vjust = -1, size = 12)) + 
  theme(axis.title.y = element_text(vjust = 0, size = 10))
#smith.load
ggsave("smith.loadLAI.ang.jpg", plot = smith.load.LAI, width = 3, height = 3)
```

Figure A2b
```{r LAI by biomass}
LAI_biomass <- ggplot(ang5, aes(x = final_dry_g.y, y = LAI)) + 
  geom_point() +
  scale_y_continuous(name = expression(paste("LAI (", cm^2, ")")), limits = c(0, 2500)) + 
  scale_x_continuous(name = "Zostera dry weight (g)", limits = c(0, 5)) +
  theme_bw() + 
  theme(panel.grid = element_blank()) + 
  geom_text(label = "LAI = 442.71*X + 99.13", x = 1.4, y = 2400, size = 3) +
  theme(axis.title.y = element_text(vjust = -1, size = 12))

ggsave("LAI_biomass.jpg", plot = LAI_biomass, width = 3, height = 3)

modLAI <- lm(ang5$LAI~ang5$final_dry_g.y)
summary(modLAI)
anova(modLAI)
```
...
```{r spatial patterns hurdle model smithora load}
